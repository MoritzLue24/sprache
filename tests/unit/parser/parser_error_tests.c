...
static void recover_does_not_loop_on_missing_operand()
{
    ...
    parsed_init(&p, "fn main() -> uint8 {2+*1;}");
...
{
    struct Parsed p;
    parsed_init(&p, "fn a() -> uint8 { ; } fn b() -> uint8 { ; }");
...
{
    struct Parsed p;
    // col 31 is the '}' that should have been a ';'
    parsed_init(&p, "fn main() -> uint8 { return a }");

    TST_ASSERT_EQ((size_t)1, p.dl.count);
    TST_ASSERT_EQ(DIAG_DIFFERENT_TOKEN_EXPECTED, p.dl.items[0].code);
    TST_ASSERT_EQ(1u, p.dl.items[0].loc.line);
    TST_ASSERT_EQ(31u, p.dl.items[0].loc.col);
...
{
    struct Parsed p;
    parsed_init(&p, "fn main() -> uint8 { 1 + ; return 2; }");
...
static void recover_keeps_node_when_only_separator_is_missing()
{
    ...
    parsed_init(&p, "fn main() -> uint8 { var a: int8 = 2 return a; }");
...
{
    struct Parsed p;
    parsed_init(&p, "1 2 3 fn main() -> uint8 { }");
...
static void recover_does_not_swallow_next_function()
{
    ...
    parsed_init(&p, "fn a() -> uint8 { ) } fn b() -> uint8 { return 1; }");
...
static void recover_in_parameter_list_keeps_later_params()
{
    ...
    parsed_init(&p, "fn f(a: uint8, 1, b: int8) -> uint8 { }");
...
static void report_missing_return_type()
{
    struct Parsed p;
    // The '->' is not optional. Without a return type the meaning of the
    // signature is open, so parse_func_def() discards the whole function
    // and leaves resuming to parse().
...
static void report_missing_type_after_arrow()
{
    struct Parsed p;
    parsed_init(&p, "fn main() -> { return 1; }");
...
static void recover_from_missing_return_type_keeps_next_function()
{
    struct Parsed p;
    // SYNC_TOPLEVEL walks to the next 'fn', so only the broken function
    // is lost.
    parsed_init(&p, "fn a() { } fn b() -> uint8 { return 1; }");

    TST_ASSERT_EQ((size_t)1, p.dl.count);
    TST_ASSERT_EQ((size_t)1, p.root.program.nl.count);
    TST_ASSERT(strcmp(val(at(&p.root.program.nl, 0)), "b") == 0);
    TST_ASSERT_EQ(NODE_RETURN, fn_stmt(&p.root, 0, 0)->kind);

    parsed_free(&p);
}

static void report_missing_parameter_type()
{
    struct Parsed p;
    // A parameter without a type is dropped, but unlike the return type
    // it does not take the function down with it.
    parsed_init(&p, "fn f(a) -> uint8 { }");
...
static void recover_from_broken_parameter_type_keeps_later_params()
{
    struct Parsed p;
    // SYNC_PARAM stops in front of the ',', so the parameter after the
    // broken type is still parsed.
    parsed_init(&p, "fn f(a: , b: uint8) -> uint8 { }");

    TST_ASSERT_EQ((size_t)1, p.dl.count);

    const struct Node* fn = at(&p.root.program.nl, 0);
    TST_ASSERT_EQ((size_t)1, fn->func_def.params.count);
    TST_ASSERT(strcmp(val(at(&fn->func_def.params, 0)), "b") == 0);

    parsed_free(&p);
}

static void report_missing_var_type()
{
    struct Parsed p;
    parsed_init(&p, "fn main() -> uint8 { var a = 1; return 2; }");

    TST_ASSERT_EQ((size_t)1, p.dl.count);
    TST_ASSERT_EQ(DIAG_DIFFERENT_TOKEN_EXPECTED, p.dl.items[0].code);
    // SYNC_STATEMENT consumes through the ';', so the return survives.
    TST_ASSERT_EQ((size_t)1, fn_body(&p.root, 0)->block.nl.count);
    TST_ASSERT_EQ(NODE_RETURN, fn_stmt(&p.root, 0, 0)->kind);
...
static void report_missing_var_type_after_colon()
{
    struct Parsed p;
    parsed_init(&p, "fn main() -> uint8 { var a: = 1; return 2; }");
...
{
    struct Parsed p;
    parsed_init(&p, "fn main() -> uint8 { f(, 1); }");
...
{
    struct Parsed p;
    parsed_init(&p, "fn main() -> uint8 { var a: uint8 2; return 1; }");
...
int main()
{
    ...
    TST_RUN(report_missing_return_type);
    TST_RUN(report_missing_type_after_arrow);
    TST_RUN(recover_from_missing_return_type_keeps_next_function);
    TST_RUN(report_missing_parameter_type);
    TST_RUN(recover_from_broken_parameter_type_keeps_later_params);
    TST_RUN(report_missing_var_type);
    TST_RUN(report_missing_var_type_after_colon);

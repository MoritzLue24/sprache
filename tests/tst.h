...
#define TST_ASSERT_EQ(expected, actual) do { \
    ...
    if (_tst_exp != _tst_act) { \
        ...
        printf(TST_RED "%*sFAILED: \"%s\" (%s:%d)\n%*sexpected %d, got %d\n" TST_RESET, \
            2, "", tst_current_name, __FILE__, __LINE__, 4, "", \
...
#define TST_ASSERT(cond) do { \
    if (!(cond)) { \
        tst_current_failed = true; \
        printf(TST_RED "%*sFAILED: \"%s\" (%s:%d)\n%*scondition failed: %s\n" TST_RESET, \
            2, "", tst_current_name, __FILE__, __LINE__, 4, "", #cond); \
    } \
} while (0)
...
#define TST_ASSERT_STR_CONTAINS(haystack, needle) do { \
    ...
    if (strstr(_tst_hay, _tst_ndl) == NULL) { \
        ...
        printf(TST_RED "%*sFAILED: \"%s\" (%s:%d)\n%*sexpected to find \"%s\" in:\n%s\n" TST_RESET, \
            2, "", tst_current_name, __FILE__, __LINE__, 4, "", _tst_ndl, _tst_hay); \
    } \
} while (0)
...
#define TST_SUMMARY() do { \
    ...
    printf("%*srun %d, failed %d\n" TST_RESET, 2, "", tst_run_cnt, tst_failed_cnt); \
} while (0)

...
bool args_parse(struct Arena* a, struct Arguments* args, int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
            ...
            if (args->sprache_stage == SPRACHE_STAGE_INVALID) {
                fprintf(
                    stderr, "Error: Invalid sprache stage '%s'\n", argv[i]
                );
                return false;
            }
        }
    ...
    if (!str_ends_with(args->input_file, ".s")) {
    ...
    if (!args->out_file) {
        ...
        args->out_file = str_replace_last(a, args->input_file, ".s", ext);
    }
    return true;
}

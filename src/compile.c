#include <string.h>
#include "sprache/compile.h"
#include "utils/str.h"

enum SpracheStage sprache_stage_from_str(char* s)
{
    if (strcmp(strupper(s), "INVALID") == 0) return SPRACHE_STAGE_INVALID;
#define STAGE(name, str, file_ext) if (strcmp(strupper(s), str) == 0) { \
    return name; }
#include "sprache/stage.def"
#undef STAGE
    return SPRACHE_STAGE_INVALID;
}

const char* sprache_stage_get_file_ext(enum SpracheStage stage)
{
    switch (stage) {
        case SPRACHE_STAGE_INVALID: return NULL;
#define STAGE(name, str, file_ext) case name: return file_ext;
#include "sprache/stage.def"
#undef STAGE
    }
    return NULL;
}

struct CompileResult sprache_compile(struct CompileOptions options)
{
    return (struct CompileResult){};
}

void sprache_free_result(struct CompileResult* r)
{

}
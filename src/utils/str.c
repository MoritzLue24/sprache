                   ...
                   const char* new_substr)
{
    ...
    char* result      = ARENA_CALLOC_LIST(a, len_before + strlen(new_substr) + len_after + 1, char);
    
    memcpy(result, s, len_before); // copy before
    memcpy(result + len_before + strlen(new_substr), s + len_before + strlen(substr), len_after); // copy after
